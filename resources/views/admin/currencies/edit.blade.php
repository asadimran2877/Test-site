...
@section('head_style')
    ...
    <link rel="stylesheet" href="{{ asset('public/backend/bootstrap-toggle/css/bootstrap-toggle.min.css') }}">
     ...
     <!-- bootstrap-select -->
    <link rel="stylesheet" href="{{ asset('public/backend/bootstrap-select-1.13.12/css/bootstrap-select.min.css')}}">
    ...
    <style>
        ...
        .dropdown.bootstrap-select.show-tick.form-control.remittance_type.bs3 {
        ...
        .dropdown.bootstrap-select.show-tick.form-control.remittance_payout_method.bs3 {
            padding: unset !important;
        }
    </style>
    @endif
                ...
                <form action="{{ url(\Config::get('adminPrefix') . '/settings/edit_currency/' . $currency->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data" id="edit_currency_form">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                         ...
                         @if (config('remittance.is_active'))
                            @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'view_remittance'))
                                <!-- Remittance Type -->
                                <div class="form-group" id="remittance-type-div">
                                    <label class="col-sm-3 control-label" for="inputEmail3">Remittance Type</label>
                                    <div class="col-sm-6">
                                        <select class="form-control remittance_type" name="remittance_type[]"
                                            id="remittance_type" multiple>
                                            <option value='send'>Send</option>
                                            <option value='receive'>Receive</option>
                                        </select>
                                    </div>
                                </div>
                                <!-- Remittance Payout Method -->
                                <div class="form-group" id="remittance-payout-method-div">
                                    <label class="col-sm-3 control-label" for="inputEmail3">Remittance Payout Method</label>
                                    <div class="col-sm-6">
                                        <select class="form-control remittance_payout_method" name="remittance_payout_method[]" id="remittance_payout_method" multiple>
                                            @foreach($remittancePayoutMethod as $key => $value)
...
@push('extra_body_scripts')
    ...
    <script type="text/javascript">
        ...
        function exchangeRateFrom(type) {
            if (type == 'crypto') {
                $('#exchange_from_div, #exchange_rate_div, #remittance-type-div, #remittance-payout-method-div').css('display', 'none');
                $('#address_div').css('display', 'block');
                $('#allowed_wallet_creation_div').css('display', 'none');
            } else if (type == 'fiat') {
                $('#exchange_from_div, #exchange_rate_div, #remittance-type-div, #remittance-payout-method-div').css('display', 'block');
                $('#address_div').css('display', 'none');
    ...
    var remittanceIsActive = "{!! config('remittance.is_active') !!}";
    if (remittanceIsActive) {
        $(window).on('load',function()
        {        
            let remittance_type = '{{ isset($remittanceType['remittance_type']) ? $remittanceType['remittance_type'] : '' }}';
            let remittancePayoutMethodId = '{{ isset($remittanceType['remittance_payout_method_id']) ? $remittanceType['remittance_payout_method_id'] : '' }}';
            if (remittance_type != 'none') {
                $.each(remittance_type.split(","), function(i,e) {
                    $("#remittance_type option[value='" + e + "']").prop("selected", true);
                    $('#remittance_type').selectpicker('refresh');
                });
            }
            if (remittancePayoutMethodId != 'none') {
                $.each(remittancePayoutMethodId.split(","), function(i,e) {
                    $("#remittance_payout_method option[value='" + e + "']").prop("selected", true);
                    $('#remittance_payout_method').selectpicker('refresh');
                });
            }
        });
    }
