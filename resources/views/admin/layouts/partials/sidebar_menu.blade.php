<ul class="sidebar-menu mt-20">
    ...
    @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'view_user') || Common::has_permission(\Auth::guard('admin')->user()->id, 'view_admins'))
        ...
        <li <?= $menu == 'users' ? ' class="active treeview"' : 'treeview'?> >
            ...
            <ul class="treeview-menu">
                ...
                @if(isActive('Agent') && Common::has_permission(\Auth::guard('admin')->user()->id, 'view_agent'))

                    <li <?= isset($sub_menu) && $sub_menu == 'agents_list' ? ' class="active"' : ''?> >

                        <a href="{{ url(\Config::get('adminPrefix').'/agents') }}">

                            <i class="fa fa-user-circle-o"></i><span>{{ __('Agents') }}</span>
    ...
    <!-- Remittances -->
    @if(config('remittance.is_active') && Common::has_permission(\Auth::guard('admin')->user()->id, 'view_remittance'))
        @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'view_remittance'))
            <li <?= isset($menu) && $menu == 'remittances' ? ' class="active"' : ''?> >
                <a href="{{ url(\Config::get('adminPrefix').'/remittances') }}"><i class="fa fa-exchange"></i><span>Remittance</span></a>
            </li>
        @endif
    @endif
    ...
    @if( isActive('Agent') && Common::has_permission(\Auth::guard('admin')->user()->id, 'view_agent'))
        ...
        <li <?= isset($menu) && $menu =='agents_revenues' ? ' class="active"' : ''?> >

            <a href="{{ url(\Config::get('adminPrefix').'/agents/revenues/list') }}">

                <i class="fa fa-address-book"></i><span>Agent Revenues</span>
