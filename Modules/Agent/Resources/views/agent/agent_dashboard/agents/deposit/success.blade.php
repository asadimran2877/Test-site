...
@section('content')
    <section class="min-vh-100" id="depositSuccess">
        <div class="my-30">
                                ...
                                <div class="bg-secondary mt-5 shadow p-35">
                                    <div>
                                        ...
                                        <div class="text-center">
                                            <p class="mt-2">{{ __('Deposit completed successfully') }}</p>
                                        </div>
                                        <p class="text-center font-weight-600 mt-2">{{ __('Deposit Amount : ') }}{{ moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal, $transaction->currency_id)) }}</p>

                                        <div class="mt-4">
                                            <div class="text-center">
                                                <a href="{{ url('agent/deposit-money/print/' . $transaction->id) }}" target="_blank" class="btn btn-grad mr-2 mt-4"><strong>{{ __('Print') }}</strong></a>
                                                <a href="{{ url('agent/deposit') }}" class="btn btn-primary ml-2 mt-4"><strong>{{ __('Deposit Money Again') }}</strong></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
