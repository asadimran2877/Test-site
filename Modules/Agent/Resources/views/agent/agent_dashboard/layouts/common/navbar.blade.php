    ...
    <li class="nav-item dropdown position-relative">
        <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            @if(!empty(Auth::guard('agent')->user()->picture))
                <img src="{{ url('public/images/agents/profile/'.Auth::guard('agent')->user()->picture) }}" class="w-30p rounded-circle" id="profileImageHeader">
            @else
                <i class="fas fa-user text-20 pt-1"></i>
            @endif

            <p class="side-text my-0 ml-2">{{ Auth::guard('agent')->user()->first_name .' '.  Auth::guard('agent')->user()->last_name}}</p>
        </a>
        ...
        <div class="dropdown-menu profile mt-2 p-0 pb-3 dropdown-menu-right">
            <div class="bg-body">
                <a class="dropdown-item py-3 d-flex align-items-center">
                    @if(!empty(Auth::guard('agent')->user()->picture))
                        <img src="{{ url('public/images/agents/profile/'.Auth::guard('agent')->user()->picture) }}" class="w-30p rounded-circle" id="profileImageHeaderdrop">
                    @else
                        <i class="fas fa-user text-20 pt-1"></i>
                    @endif
            ...
            <a class="dropdown-item mt-2" href="{{url('agent/profile')}}">
            ...
            <a class="dropdown-item mt-2" href="{{ url('agent/logout') }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-30 w-30" fill="none" viewBox="0 0 24 24" stroke="currentColor">
