...

{
...

    {
        ...
        if (module('Remittance')) {
            array_push(self::$transactionTypes, Remittance);
        }
    ...
    {
        return $this->belongsTo(\Modules\Remittance\Entities\Remittance::class, 'transaction_reference_id', 'id');
    }
...

    {
...

        {
...

            {
                ...
                } elseif (module('Remittance') && $type == Remittance ) {
                    
                    $whereInCondition = [Remittance];
...

    {
        ...
        if (module('Agent')) {
            ...
            $transactions = $transactions->with('agent:id,first_name,last_name');
...

    {
...

        {
            ...
            if (module('Agent') && Request::segment(2) == 'agents') {

                $transactions->where(function ($q) use ($user) {

                    $q->where(['transactions.agent_id' => $user]);
    ...
    public function getRevenuesList($from, $to, $currency, $type, $agent = null)

    {
        ...
        if (module('Agent')) {
            ...
            array_push($selectOptions, 'transactions.agent_id', 'transactions.agent_percentage');

            $revenues = $revenues->with('agent:id,first_name,last_name')->where(function ($query) {

                $query->where('transactions.agent_id', '!=', null);
        ...
        if (module('Agent') && !empty($agent)) {

            $revenues->where(function ($q) use ($agent) {

                $q->where(['transactions.agent_id' => $agent]);
...

    {
        ...
        public function getTransactionsUsersAgentsName($user, $type)

    {

        $getUserEndUserTransaction = $this->where(function ($q) use ($user) {

            $q->where(['user_id' => $user])->orWhere(['agent_id' => $user]);
        ...
        $userTransaction = $getUserEndUserTransaction->with(['user:id,first_name,last_name', 'agent:id,first_name,last_name'])->first(['user_id', 'agent_id']);
        ...
        if (!empty($userTransaction)) {
            ...
            if ($userTransaction->agent_id == $user) {

                return $userTransaction->agent;
    ...
    public function getTransactionsAgentResponse($search, $type)

    {
        ...
        $getTransactionsEndUsers = $this->whereHas('agent', function ($query) use ($search) {
        ...
        $getTrxEndUsers = $getTransactionsEndUsers->with(['agent:id,first_name,last_name'])->get(['agent_id'])->map(function ($transactionB) {

            $arr['agent_id'] = $transactionB->agent_id;

            $arr['first_name'] = $transactionB->agent->first_name;

            $arr['last_name'] = $transactionB->agent->last_name;
    ...
    public function getAgentTransactions($from, $to, $type, $wallet, $status)

    {
        ...
        $conditions['transactions.agent_id'] = Auth::guard('agent')->user()->id;
    ...
    public function dashboardAgentTransactionList($user_id)

    {

        $transaction = Transaction::where(['agent_id' => $user_id])->orderBy('id', 'desc')->take(10)->get();
    ...
    public function agentRevenue($user_id)
