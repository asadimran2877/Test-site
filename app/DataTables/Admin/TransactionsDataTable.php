            ...
            ->addColumn('sender', function ($transaction) {
                ...
                switch ($transaction->transaction_type_id) {
                    ...
                    case (module('CryptoExchange') ? Crypto_Exchange : false):
                        ...
                        } elseif (module('CryptoExchange') && isset($transaction->crypto_exchange->email_phone) && !empty($transaction->crypto_exchange->email_phone)) {

                           $senderWithLink = isset($transaction->crypto_exchange->email_phone) ?  $transaction->crypto_exchange->email_phone : '' ;

                        } elseif (module('Agent') && isset($transaction->agent->first_name) && !empty($transaction->agent->first_name)) {

                            $sender = $transaction->agent->first_name.' '.$transaction->agent->last_name;

                            $url = isActive('Agent') ? '<a href="' . url(Config::get('adminPrefix').'/agents/edit/' . $transaction->agent_id) . '">'.$sender.'</a>' : $sender;

                            $senderWithLink = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_agent')) ? $url : $sender;
