...

{
...

    {
        ...
        if ($type == Deposit || $type == Withdrawal || $type == Remittance || $type == 'all')
...

        {
...

            {
                ...
                case (module('Remittance') ? Remittance : false):
                    ...
                    $data['html'] = "<div class='d-flex justify-content-between flex-wrap mt-2'>" .
                        "<div> <p>" . __('Remittance Pay Currency') . "</p></div>" .
                        "<div> <p>" . $transaction->currency->code . "</p></div>" .
                        "</div>" .
                        ...
                        "<div class='d-flex justify-content-between flex-wrap  mt-4'>" .
                        "<div> <p>" . __('Remittance Send Amount') . "</p></div>" .
                        "<div><p>" . moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal)) . "</p></div>" .
                        "</div>";
                    ...
                    $data['total'] =  "<div class='text-center '>
                                                <h2 class='text-white text-center font-weight-700 text-20'>" . __('Remittance Amount') . "</h2>
                                                <h1 class='text-white mt-4'><strong>" . moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal)) . "</strong></h1>
                                            </div>" .

                        "<h4 class='mt-2 text-center text-white text-16'>" . $transaction->created_at->format('jS F Y') . "</h4>" .
                        "<div class='form-group mt-5 text-center'>" .
                        "<a href='" . url('remittance/remittance-money/print/' . $transaction->id) . "' target='_blank' class='btn btn-light pl-4 pr-4 btn-sm'>" . __('message.dashboard.vouchers.success.print') . "</a>" .
                        "</div>";
                    break;
            }
