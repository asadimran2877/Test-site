...

{
...

    {
        ...
        if ($tab == 'deposit') {
            ...
            $data['payment_methods'] = PaymentMethod::with(['fees_limit' => function ($q) use ($transaction_type, $currency_id)
            {
                ...
                ->whereNotIn('id', [1, 4, 5, 6, 7, 8, 9, 10]) // ['paymoney', '2Checkout', 'PayUMoney', 'Coinpayments', 'Payeer', 'BlockIo'] respectively
                ->where(['status' => 'Active'])
                ->get(['id', 'name']);
            return view('admin.feeslimits.deposit_limit', $data);
...

    {
        ...
        if ($request->transaction_type == 1 || $request->transaction_type == 2 || $request->transaction_type == 11) {
            ...
            foreach ($payment_method_id as $key => $value) {
                ...
                if (empty($feeslimit)) {
                    ...
                    if (isActive('Agent') && ($value == Cash) && isset($agent_percentage)) {

                        $feeslimit->agent_percentage = $agent_percentage[0];
...

    {
        ...
        if ($transaction_type == Deposit) {
            ...
            $feeslimit = PaymentMethod::with(['fees_limit' => function ($q) use ($transaction_type, $currency_id)
            {
                ...
                ->whereNotIn('id', [1, 4, 5, 6, 7, 8, 9, 10])
                ->where(['status' => 'Active'])
                ->get(['id', 'name']);
