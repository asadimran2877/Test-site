...

{
    ...
    public static $templateIds = ['deposit' => 23, 'payout' => 24, 'exchange' => 25, 'send' => 26, 'request' => 27, 'payment' => 28, 'remittance' => 33, 'crypto-exchange' => 34];
...

        {
...

            {
                ...
                else if ($type == 'remittance')
                {
                    ...
                    $message = str_replace('{created_at}', dateFormat($data->created_at, $data->user_id), $message);
                    $message = str_replace('{user}', $data->sender->first_name . ' ' . $data->sender->last_name, $message);
                    $message = str_replace('{amount}', moneyFormat($data->currency->symbol, formatNumber($data->total)), $message);
                    $message = str_replace('{transferred_amount}', moneyFormat($data->currency->symbol, formatNumber($data->transferred_amount)), $message);
                    $message = str_replace('{code}', $data->currency->code, $message);
                    $message = str_replace('{fee}', moneyFormat($data->currency->symbol, formatNumber($data->fees)), $message);
